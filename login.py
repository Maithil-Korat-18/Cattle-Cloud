...
def send_verification_code():
    ...
    if send_otp_email(email, otp, "registration"):
        ...
        print(f"[DEV NOTICE] Email sending failed or SMTP credentials not configured. Generated OTP for {email}: {otp}")
        return jsonify({"success": True, "dev_otp": otp, "message": "Verification code generated. Check server log or dev_otp if email server is unconfigured."})
...
def send_reset_code():
    ...
    if send_otp_email(email, otp, "reset"):
        ...
        print(f"[DEV NOTICE] Reset email failed or SMTP credentials not configured. Generated Reset OTP for {email}: {otp}")
        return jsonify({"success": True, "dev_otp": otp, "message": "Reset code generated. Check server log or dev_otp if email server is unconfigured."})
